...
void WB()
{
	...
	function = (MEM_WB.IR & 0x3f);
		...
		rt = MEM_WB.RegisterRt;

		rd = MEM_WB.RegisterRd;
...
void MEM()
{
	...
	if(ENABLE_FORWARDING){
		if(MEM_WB.RegWrite && (MEM_WB.RegisterRd != 0) && !(EX_MEM.RegisterRt && (EX_MEM.RegisterRd != 0) && (EX_MEM.RegisterRd == ID_EX.RegisterRs)) && (MEM_WB.RegisterRd == ID_EX.RegisterRs)){
			ID_EX.A = MEM_WB.ALUOutput;
			ForwardA = 1;
		} 
		if(MEM_WB.RegWrite && (MEM_WB.RegisterRd != 0) && !(EX_MEM.RegisterRs && (EX_MEM.RegisterRd != 0) && (EX_MEM.RegisterRd == ID_EX.RegisterRt)) && (MEM_WB.RegisterRd == ID_EX.RegisterRt)){
...
void ID()
{
	if(stallcounter == 0){
		...
		if(opcode == 0x00 && !(ForwardA || ForwardB)){
			ID_EX.RegWrite = 1;
			ID_EX.RegDst = 1;
		}
		else if(!(ForwardA || ForwardB)){
			switch(opcode){
				case 0x08: //ADDI
				case 0x09: //ADDIU
				case 0x0C: //ANDI
		...
		//sets destination to RD or RT register
		if(ID_EX.RegDst == 1)
			ID_EX.RegisterRd = rd;
		else if(ID_EX.RegDst == 0)
			ID_EX.RegisterRd = rt;
		...
		/*	else{
				if(ForwardA == 01){
					...
					ID_EX.A = MEM_WB.ALUOutput;
					//ID_EX.RegisterRs = MEM_WB.RegisterRs;
				...
				if(ForwardB == 01){
					...
					ID_EX.B = MEM_WB.ALUOutput;
					//ID_EX.RegisterRt = MEM_WB.RegisterRt;
		...
		(MEM_WB.RegWrite && (MEM_WB.RegisterRd != 0) && (MEM_WB.RegisterRd == ID_EX.RegisterRs))){
		//	if(!ENABLE_FORWARDING){
		...
		/*	else{
				if(ForwardA == 10){
					...
					ID_EX.A = EX_MEM.ALUOutput;
			//		ID_EX.RegisterRs = EX_MEM.RegisterRs;
...
void IF()
{
	if(stallcounter == 0) pause_IF = FALSE;
	...
	else NEXT_STATE.PC = CURRENT_STATE.PC;
	if(stallcounter > 0){
		--stallcounter;
		pause_IF = TRUE;
	}
...
uint32_t instruction, opcode, function, rs, rt, rd, sa, immediate, target;
    ...
    char reg_name[32][6] = {"$r0", "$r1", "$r2", "$r3", "$r4","$r5", "$r6", "$r7", "$r8","$r9", "$r10", "$r11", "$r12", "$r13", "$r14", "$r15", "$r16", "$r17", "$r16", "$r19", "$r20", "$r21", "$r22", "$r23", "$r24", "$r25", "$r26", "$r27", "$r28", "$r29", "$r30", "$r31"};//{"$zero", "$at", "$v0", "$v1", "$a0","$a1", "$a2", "$a3", "$t0","$t1", "$t2", "$t3", "$t4", "$t5", "$t6", "$t7", "$s0", "$s1", "$s2", "$s3", "$s4", "$s5", "$s6", "$s7", "$t8", "$t9", "$k0", "$k1", "$gp", "$sp", "$fp", "$ra"};
...
void show_pipeline(){
	...
	//printf("%d\n", stallcounter);
	//printf("%d|%d  %d %d %d\n", ID_EX.RegDst, ID_EX.RegWrite,ID_EX.RegisterRd, ID_EX.RegisterRs, ID_EX.RegisterRt);
	//printf("  %d  %d\n", EX_MEM.RegisterRd, MEM_WB.RegisterRd);
